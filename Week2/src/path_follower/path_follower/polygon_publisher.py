...
class PolygonPublisher(Node):
    def __init__(self):
        ...
        #self.side_time = 3.0
        self.side_time = self.distance / self.linear_velocity #tiempo estimado para que cada lado sea de 2m 
    ...
    def parameters_callback(self, params):
        for param in params:
            if param.name == 'sides':
                ...
                min_figure_time = (self.angle/1.5) * (self.sides -1) + (self.distance / 0.2) * self.sides
                if (param.value < min_figure_time):
                    ...
                    total_turn_time = self.turn_time *(self.sides -1)
